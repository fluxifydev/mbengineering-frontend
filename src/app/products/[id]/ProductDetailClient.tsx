...
export default function ProductDetailClient({ id }: ProductDetailClientProps) {
  ...
  const [relatedProducts, setRelatedProducts] = useState<RenderProduct[]>([]);
            ...
            relSnap.forEach((d) => {
              if (d.id !== docSnap.id) {
                ...
                list.push({
                  ...
                  specifications: rData.specifications || [],
                  brochureUrl: rData.brochureUrl || '',
                });
              }
            });
    ...
    <>
      ...
      <main className="pt-28 pb-16 md:pt-32 md:pb-24 max-w-[1440px] mx-auto px-margin-mobile md:px-margin-desktop space-y-12">
        ...
        {relatedProducts.length > 0 && (
          <div className="pt-12 border-t border-outline-variant/50 space-y-6">
            <h2 className="font-display text-xl sm:text-2xl font-bold text-primary tracking-tight">
              Related Machinery Models
            </h2>
              ...
              {relatedProducts.map((relProduct) => (
                <div key={relProduct.id} className="flex">
                  <div 
                    ...
                    className="bg-white rounded-xl border border-outline-variant/60 overflow-hidden shadow-sm hover:shadow-md hover:border-primary/50 transition-all duration-300 w-full flex flex-col justify-between h-[360px] cursor-pointer group"
                  >
                    <div className="h-40 overflow-hidden relative bg-slate-100/70 flex items-center justify-center text-slate-400 group-hover:bg-slate-100 transition-colors shrink-0">
                      {relProduct.imageUrl ? (
                        <Image 
                          alt={relProduct.name}
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between">
                      <div className="space-y-1.5">
                        <h4 className="font-display text-sm sm:text-base font-bold text-primary truncate">
                          {relProduct.name}
                        </h4>
                      ...
                      <div className="pt-3 font-bold text-xs text-accent group-hover:underline flex items-center gap-1">
